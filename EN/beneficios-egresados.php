<!DOCTYPE html>
<html lang="en">
<head>
	<?php include_once 'head.php' ?>
	<title>Colegio Jefferson | Admission Benefits for Our Alumni</title>
</head>
<body>
	<?php include_once 'tagMannager.php' ?>
	<div class="contBox">

		<?php include_once 'menu.php' ?>

		<div class="bannerSections">
			<div class="titleBannerSection simple">
				<h1>Admission Benefits for Our Alumni</h1>
			</div>
        </div>

        <div class="beneficiosAdmision fullW">

            <div class="introTextImg">
				<img src="../img/beneficios-egresados-img3.jpg" alt="">
			</div>

            <div class="VideoYoutube fullwidth beneficios">
                <iframe src="https://www.youtube.com/embed/eA0dozN3SI4" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>
            </div>
        </div>
        
		<?php include_once 'footer.php' ?>

	</div>

</body>
</html>